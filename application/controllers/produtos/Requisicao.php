...
class Requisicao extends MY_Controller 
{
     ...
     */
    public $modulo  = 'requisicao';

    public $url     = 'requisicao-de-produtos';
    ...
    {       
        ...
        $this->model = $this->Requisicao_model;
    ...
    public function listar()
    {
         ...
         */
        $page_data['lista']         = $this->model->listar_requisicoes(); 
        ...
        if (count($page_data['lista']) >= 1)
        {
             ...
             * View Empty
             */
            $page_data['page'] = 'produtos/requisicao/main-list';
        }
        else
        {
            /**
             * View 
             */
            $page_data['page'] = 'produtos/requisicao/index';
        } 
   ...
   public function cadastrar()
   {
         ...
         * 
         */ 
        $page_data['unidades_empresas'] = $this->model->listar_unidades_empresa(); 

        /**
         * Unidades da Empresa
         * 
         */ 
        $page_data['estoque_origem'] = array(''); 

        /**
         * Busca os itens da Requisicao 
         */
        $page_data['itens_requisicao'] = array('');


        /**
         * View 
         */
        $page_data['page']          = 'produtos/requisicao/form';
   ...
   public function editar()
   {
        ...
        if(!$id)
        {
             ...
             */ 
            $this->log_zata->log_error('Danger','Acesso indevido: Requisicao->editar() --> Parâmetro ID nao numerico');
        ...
        else // Se o parametro esta CORRETO - If the parameter is CORRECT
        {
            ...
            if (!$this->model->getRow($id))
            {
                 ...
                 */
                $this->log_zata->log_error('Error','Acesso invalido: Requisicao->editar() --> O registro nao foi encontrado no banco de dados. ');
            ...
            else
            {
                 ...
                 * 
                 */ 
                $page_data['estoque_origem']    = $this->model->listar_estoques_unidade($page_data['show']->id_unidade); 

                /**
                 * Busca os itens da Requisicao 
                 */
                $page_data['itens_requisicao']  = $this->model->Produtos_model->itens_requisicao( $page_data['show']->id_requisicao_produto);
   ...
   public function salvar()
   {
        ...
        /**
         * Alterar Status da Requisicao e setar o redirecionamento da pagina de pois da alteracao
         */
        if( $this->input->post('id') != NULL){
            //Verifica qual botao de acao esta sendo acionado
            switch ($this->input->post('acao'))
            {
                // Criar Requisicao
                case 'criar':
                    $link_url = $this->url.'/editar/'.$this->input->post('id');
                    $sit_status_requisicao = 'P'; //[P]producao
                    break;

                // Salvar alteracoes da antes de enviar para aprovacao     
                    case 'salvar':
                    $link_url = $this->url.'/editar/'.$this->input->post('id'); 
                    $sit_status_requisicao = 'P';//[P]producao
                    break;

                // Enviar Requisicao para Aprovacao do Gerente    
                    case 'enviar':
                    $link_url = $this->url.'/listar';  
                    $sit_status_requisicao = 'G';//[G]gerente  
                    break;

                // Requisicao aprovada, liberar para estoque realizar a entrega
                    case 'aprovar':
                    $link_url = $this->url.'/listar';  
                    $sit_status_requisicao = 'E';//[G]gerente  
                    $dth_aprovacao = date('Y-m-d');
                    break;

                    case 'recusar':
                    $link_url = $this->url.'/listar';  
                    $sit_status_requisicao = 'P';//[G]gerente  
                    break;

                // Estoque entregou mercadoria   

                    case 'entrega-concluida':
                    $link_url = $this->url.'/listar';  
                    $sit_status_requisicao = 'F';//[G]gerente 
                    $dth_entrega = date('Y-m-d'); 
                    break;       
            ...
            {
            // Entra aqui se o formulario estiver cadastrado uma nova requisicao
            $sit_status_requisicao = 'P'; //[P]producao
            // Estou colocando ''[vazio], para identificar que o formulario esta em modo de cadastro, irei utilizala mas abaixo...
            $link_url = '';  
        }
         ...
         */
        $data = array(
            "num_requisicao"                 => $this->gera_numero_requisicao($this->input->post('num_requisicao')),
            "id_unidade"                     => (int)$this->input->post('id_unidade'),
            "id_origem_estoque"              => (int)$this->input->post('id_origem_estoque'),
            "id_destino_estoque"             => 2,
            "tipo_destino"                   => $this->input->post('tipo_destino'),
            "data_emissao"                   => gravaData($this->input->post('data_emissao')),
            "data_necessidade_entrega"       => gravaData($this->input->post('data_necessidade_entrega')),
            "data_aprovacao"                 => isset($dth_aprovacao) ? $dth_aprovacao: gravaData($this->input->post('data_aprovacao')),
            "data_entrega"                   => isset($dth_entrega)? $dth_entrega: gravaData($this->input->post('data_entrega')),
            "sit_status_requisicao"          => $sit_status_requisicao
        ); 

        /**
         * sit_status_requisica 
         * Criar e alterar = P (Producao)
         * Aprovada = A (Pelo gerenge)\
         * Entrege = E (Estoque)
         */
        ...
        //Se o formulario estiver em modo de cadastro, essa variavel foi utilizada como auxiliar, ver acima
        if ($link_url == '')
        {
           $link_url = $this->url.'/editar/'.$id;  
       } 
   ...
   function excluir($id) 
   {
        ...
        else
        {
             ...
             */
            $this->log_zata->log_error('Error', 'Falha ao excluir: Requisicao->excluir() --> Nao foi possivel excluir o registro '. $id);
    ...
    * @author    Ralny Andrade | <ra@trivee.com.br> | https://github.com/ralny
    */ 
   public function ajax_combo_estoque($id_unidade){

    $lista_estoques = $this->model->listar_estoques_unidade($id_unidade);
        //Verifica se existe algum registro no banco de dados
    if ( empty ( $lista_estoques ) )
        ...
        $lista_estoques =  array (
            array("nome_estoque"=>"Nenhum assunto encontrado")
        );

    echo json_encode($lista_estoques);

    return;
}

     /**
    * Gera Numero Requisicao
    *
    * Gerar sequncial do numero da requisicao
    * 
    * @author    Ralny Andrade | <ra@trivee.com.br> | https://github.com/ralny
    */
     function gera_numero_requisicao($numero_os){

        if( $numero_os == '')
        {
            $num_os = $this->model->buscar_num_requisicao();
     ...
     * @author    Ralny Andrade | <ra@trivee.com.br> | https://github.com/ralny
     */
     public function aprovar_requisicao(){

        $id = $this->input->post('idRequisicao');

        $data = array(
            "sit_status_requisicao" => 'E',
         ...
         * Atualizando 
         */
        if($this->db->update('pro_requisicao_produto',$data) == true){
     ...
     * @author    Ralny Andrade | <ra@trivee.com.br> | https://github.com/ralny
     */    
     public function print_requisicao()
     {
        /**
         * Responsável por registrar o acesso do usuario no metodo do controller 
         * Responsible for registering user access in controller method
        ...
        if(!$id)
        {
             ...
             * Logging the error log to the database
             */ 
            $this->log_zata->log_error('Danger','Acesso indevido: Requisicao->print_requisicao() --> Parâmetro ID nao numerico');
        ...
        else // Se o parametro esta CORRETO - If the parameter is CORRECT
        {
            ...
            if (!$this->model->getRow($id))
            {
                 ...
                 * Writing the Error Log to the Database
                 */
                $this->log_zata->log_error('Error','Acesso invalido: Requisicao->print_requisicao() --> O registro nao foi encontrado no banco de dados. ');
            ...
            else
            {
                 ...
                 * View 
                 */
                $page_data['page']          = 'relatorios/produtos/requisicao/print_requisicao';
                 ...
                 * Loading everything on the view
                 */
                $this->load->view('relatorios/main_a4', $page_data);
