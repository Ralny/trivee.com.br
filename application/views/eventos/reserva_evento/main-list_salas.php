			...
			<div class="portlet-title">
				...
				<div class="actions btn-set">
					<div class="form-actions top">
						<button id="add_sala" class="btn btn-success" data-toggle="modal" href="#basic"><i class="fa fa-plus"></i> Adicionar Sala</button>
					</div>
				</div>
			</div>
				...
				<div class="row">
					...
					foreach ($lista_reserva_salas as $sala):
						...
						<tr>
							...
							<td>
								<?= isset($sala->desc_formato_de_sala) ? $sala->desc_formato_de_sala : null ;?>
							</td>
							<td>
							...
							<td>
								<?= isset($sala->dth_fim) ? mostraData($sala->dth_fim) : null ;?>
								<span class="label label-warning"> 
									<?= isset($sala->hora_fim) ? exibir_hora($sala->hora_fim,'') : null ;?>
								</span>
							</td>
							
							<td>
								<?= isset($sala->pax_garantidas) ? $sala->pax_garantidas : null ;?>
							</td>
							<td>
								<?= isset($sala->valor_total) ? moeda($sala->valor_total) : null ;?>
							</td>
							<td>
								<?= isset($sala->valor_total_taxas) ? moeda($sala->valor_total_taxas) : null ;?>
							</td>
							<td>
						...
						<tr style="text-align: right; font-size: 16px; background-color:#FCF8E3">
							<td colspan="9" style="border-top: 0px;">
								<strong>Valor total: <?=  moeda($valor_total) ?></strong>
							</td>
						</tr>

						<tr style="text-align: right; font-size: 16px; background-color:#FCF8E3;">
							<td colspan="9" style="border-top: 0px;">
								<strong>ISS: <?=  moeda($valor_total_com_taxas - $valor_total) ?></strong>
							</td>
						</tr>

						<tr style="text-align: right; font-size: 16px; background-color:#FCF8E3">
							<td colspan="9"  style="border-top: 0px;">
								<strong>Total de pagamento: <?=  moeda($valor_total_com_taxas) ?></strong>
							</td>
						</tr>
