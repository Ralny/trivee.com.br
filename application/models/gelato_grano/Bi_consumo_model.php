...
class Bi_consumo_model extends MY_Model {
    ...
    */
    ...
    public function requisicao_de_item($id, $dth_inicial, $dth_final) {
        ...
        $sql = "
                ...
                WHERE
                  r.data_entrega BETWEEN ('$dth_inicial') AND ('$dth_final') AND
                  i.id_produto = $id AND
    ...
    public function total_gelato_produzido() {
 
        $sql = "
                
                SELECT
                  SUM(c.peso) AS total_produzido
                FROM
                  gelato_consumo_itens_receita c
                WHERE
        ...
        if ($query->result()){
            ...
            if ($query->row()->total_produzido >= 1000)
            {
                $peso = number_format($query->row()->total_produzido,0,".",","). ' KG';
            }
            else
            {
                $peso = $query->row()->total_produzido. ' G';
            }
