...
<?php echo $this->session->flashdata('error_upload') ?>	
	<div class="col-md-6">
		<form action="<?= base_url() ?>zata/upload/do_upload_img" role="form" method="post" enctype="multipart/form-data">
		<input type="hidden" name="token_id" value="<?=  isset($token_id) ? $token_id : null ;?>">
		<input type="hidden" name="tabela_db" value="<?=  isset($tabela_db) ? $tabela_db : null ;?>">
